...
export const bcfOWLPrefixes ="PREFIX bcfOWL:<http://lbd.arch.rwth-aachen.de/bcfOWL#>\n\n"
export const geoPrefix = "PREFIX geo: <http://www.opengis.net/ont/geosparql#>\n\n"
...
class BcfOWLService {
    ...
    async  getViepointOriginatigDocument(viewpointGUID: string) {
        let viewpoint_uri="<"+base_uri+"/graph/"+this.project_id+"/"+viewpointGUID+"/>"
        let urlencoded = new URLSearchParams();
        urlencoded.append("query", bcfOWLPrefixes+"select ?fn ?durl ?location ?rotation ?scale\n" +
            "WHERE {\n" +
            "  "+viewpoint_uri+"  bcfOWL:hasOriginatingDocument ?og.\n" +
        ...
        const response = await fetch(base_uri+"/sparql/"+this.project_id+"/query", {method: 'POST', headers: this.myHeaders, body: urlencoded, redirect: this.follow});
        if (!response.ok) {
    ...
    async  getCurrentProject1() {
        let urlencoded = new URLSearchParams();
        urlencoded.append("query", bcfOWLPrefixes +
            "CONSTRUCT {?s ?p ?o}\n" +
            "WHERE {\n" +
            "?s a bcfOWL:Project ;\n" +
            "?p ?o .\n" +
        ...
        const response = await fetch(base_uri+"/graph/"+this.project_id+"/query", {method: 'POST', headers: this.myHeaders, body: urlencoded, redirect: this.follow});
        if (!response.ok) {
    ...
    async  describeUser(uri: string) {
        ...
        const response = await fetch(base_uri+"/graph/users/query", {method: 'POST', headers: this.myHeaders, body: urlencoded, redirect: this.follow});
        if (!response.ok) {
            const message = `getCurrentProject: An error has occurred: ${response.status}`;
            NotificationManager.warning(message, "Error", 3000);
            throw new Error(message);
    ...
    async  getUserByEmail(email: string) {
        ...
        urlencoded.append("query", "CONSTRUCT {?subject ?predicate ?object }\n" +
            "WHERE {\n" +
            "  ?subject <http://xmlns.com/foaf/0.1/mbox> <mailto:"+email+"> .\n" +
    ...
    async  getAll() {
        ...
        let urlencoded = new URLSearchParams();
        urlencoded.append("query", "CONSTRUCT {?s ?p ?o}\n" +
            "WHERE \n" +
            "{  ?s ?p ?o .\n" +
            "  FILTER(!IsLiteral(?o)) .\n" +
            "} \n");
    ...
    async  getMINTopicDate() {
        if(!this.project_id)
            alert("Project not selected. ")
        let urlencoded = new URLSearchParams();
        urlencoded.append("query", "PREFIX value:    <http://www.value.de>\n" +
    ...
    async  getTopicByGUID(guid: string) {
        ...
        urlencoded.append("query", "CONSTRUCT {?subject ?predicate ?object}\n" +
            "WHERE {\n" +
            "  ?subject <http://lbd.arch.rwth-aachen.de/bcfOWL#hasGuid> \""+guid+"\" .\n" +
            "  ?subject ?predicate ?object .\n" +
            "}");
    ...
    async  getViepointCameras4Document(doc_uri: string) {
        ...
        if(!this.project_id)
            alert("Project not selected. ")
        let urlencoded = new URLSearchParams();
        urlencoded.append("query", bcfOWLPrefixes+"CONSTRUCT {?viewpoint ?p ?o}\n" +
            "WHERE {\n" +
        ...
        console.log(bcfOWLPrefixes+"CONSTRUCT {?viewpoint ?p ?o}\n" +
            "WHERE {\n" +
            "  { ?viewpoint bcfOWL:hasOriginatingDocument <"+doc_uri+"> . }\n" +
            "  UNION {?viewpoint bcfOWL:hasOriginatingDocument '"+guid+"' . }\n" +
            "   ?viewpoint a bcfOWL:Viewpoint. \n" +
    ...
    async  getTopicGuids4Document(doc_uri: string) {
        if(!this.project_id)
            alert("Project not selected. ")
        let urlencoded = new URLSearchParams();
        urlencoded.append("query", bcfOWLPrefixes+"CONSTRUCT {?topic <http://lbd.arch.rwth-aachen.de/bcfOWL#hasGuid>  ?topic_guid }\n" +
            "WHERE {\n" +
            "  ?viewpoint ?p <"+doc_uri+"> .\n" +
    ...
    async  updateSpatialRepresentation(doc_uri: string, spatrep_uri: string, spatial_representation: { alignment?: string; location: any; rotation: any; scale: any; }) {
        //TODO guid not used
        ...
        const response = await fetch(base_uri+"/graph/"+this.project_id + "/update",  {method: 'POST', headers: this.myHeaders, body: urlencoded, redirect: this.follow});
        if (!response.ok) {
    ...
    async  createDocumentWithSpatialRepresentation(file_name: string, spatial_representation: { alignment?: string; location: any; rotation: any; scale: any; }) {
