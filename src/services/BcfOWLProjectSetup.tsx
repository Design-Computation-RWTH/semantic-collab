...
class BcfOWLProjectSetup {
    ...
    async setCurrentProjectName(name: string) {
        ...
        urlencoded.append("update",
            ...
            "INSERT { ?project bcfOWL:hasName \"" + name + "\"}\n" +
            "WHERE {\n" +
            " ?project a bcfOWL:Project;\n" +
            " bcfOWL:hasName ?name .\n" +
            "}");

        fetch(base_uri + "/graph/" + this.project_id + "/update", {method: 'POST', headers: headers, body: urlencoded, redirect: this.follow})
    ...
    async insertPropertyValue(bcfProperty: string, value: string) {
        ...
        let value_uri = "<" + base_uri + "/graph/" + this.project_id + "/" + value + ">"
        ...
        urlencoded.append("update",
            "PREFIX bcfOWL:<http://lbd.arch.rwth-aachen.de/bcfOWL#>\n" +
            "\n" +
            "INSERT { ?project bcfOWL:" + bcfProperty + " " + value_uri + "}\n" +
            "WHERE {\n" +
            " ?project a bcfOWL:Project .\n" +
            "}");
    ...
    async removePropertyValue(bcfProperty: string, value: string) {
        ...
        urlencoded.append("update",
            "PREFIX bcfOWL:<http://lbd.arch.rwth-aachen.de/bcfOWL#>\n" +
            "\n" +
            "DELETE { ?project bcfOWL:" + bcfProperty + " " + value_uri + "}\n" +
            "WHERE {\n" +
            " ?project a bcfOWL:Project .\n" +
            "}");
    ...
    async insertUser(value: string) {
        ...
        urlencoded.append("update",
            "PREFIX bcfOWL:<http://lbd.arch.rwth-aachen.de/bcfOWL#>\n" +
            "\n" +
            "INSERT { ?project bcfOWL:hasUser " + value_uri + "}\n" +
            "WHERE {\n" +
            " ?project a bcfOWL:Project .\n" +
            "}");
    ...
    async deletetUser(value: string) {
        ...
        urlencoded.append("update",
            "PREFIX bcfOWL:<http://lbd.arch.rwth-aachen.de/bcfOWL#>\n" +
            "\n" +
            "DELETE { ?project bcfOWL:hasUser " + value_uri + "}\n" +
            "WHERE {\n" +
            " ?project a bcfOWL:Project .\n" +
            "}");
    ...
    async addProject(project_name: string|null) {
        ...
        fetch("https://caia.herokuapp.com/bcf/sparql/projects", {method: 'POST', headers: myHeaders, body: raw, redirect: this.follow})
