...
class BCFAPIService {
    ...
    private readonly requestGetOptions: { redirect: "error" | "follow" | "manual"; headers: Headers; method: string };

    private readonly PUTHeaders: Headers;
    private readonly requestPutOptions: { redirect: RequestRedirect; headers: Headers; method: string;body?: BodyInit };
    ...
    async  getProjects() {
        const response = await fetch(base_uri+"/bcf/3.0/projects", this.requestGetOptions);
        if (!response.ok) {
    ...
    async  getTopics() {
        const response = await fetch(base_uri+"/bcf/3.0/projects/"+this.project_id+"/topics", this.requestGetOptions);
        if (!response.ok) {
    ...
    async  getTopicViewPoints(topic_id: string) {
        const response = await fetch(base_uri+"/bcf/3.0/projects/"+this.project_id+"/topics/"+topic_id+"/viewpoints", this.requestGetOptions);
        if (!response.ok) {
            const message = `getViewPoints: An error has occured: ${response.status}`;
            NotificationManager.warning(message, "Error syncing Projects", 3000);
            throw new Error(message);
    ...
    async  getTopicComments(topic_id: string) {
        const response = await fetch(base_uri+"/bcf/3.0/projects/"+this.project_id+"/topics/"+topic_id+"/comments/", this.requestGetOptions);
        if (!response.ok) {
    ...
    async  putSpatialRepresentation(document_id: string, spatial_representation: any) {
        let requestOptions = Object.assign({}, this.requestPutOptions);
        requestOptions.body = JSON.stringify(spatial_representation);
        const response = await fetch(base_uri+"/bcf/3.0/projects/"+this.project_id+"/documents/"+document_id+"/spatial_representation", requestOptions);
        if (!response.ok) {
            const message = `getProjects: An error has occured: ${response.status}`;
            NotificationManager.warning(message, "Error syncing Projects", 3000);
            throw new Error(message);
