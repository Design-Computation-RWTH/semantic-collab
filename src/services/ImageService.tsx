...
class ImageService {
    ...
    private readonly requestOptions: { redirect: any; headers: Headers; method: string };
    ...
    async getSnapShot (topic_id: string, viewpoint_id: string) {
        ...
        const response = await fetch("https://caia.herokuapp.com/bcf/3.0/projects/"+this.project_id+"/topics/"+topic_id+"/viewpoints/"+viewpoint_id+"/snapshot", this.requestOptions);
        if (!response.ok) {
    ...
    async getThumbnailData(viewpoint_guid: string) {
        const response = await fetch(base_uri+"/files/" + this.project_id + "/" + viewpoint_guid + "_thumbnail.png", this.requestOptions);
        if (!response.ok) {
            const message = `An error has occurred: ${response.status}`;
            throw new Error(message);
        }
    ...
    async getImageData4GUID(viewpoint_guid: string) {
        console.log("read snapshot")
        const response = await fetch(base_uri+"/files/"  + this.project_id + "/" + viewpoint_guid + ".png", this.requestOptions);
        if (!response.ok) {
            const message = `An error has occurred: ${response.status}`;
            throw new Error(message);
        }
