...
class Representations extends React.Component {
  constructor(props) {
    ...
    this.un_SidebarName = PubSub.publish("SidebarName", {
      name: "Representations",
    });
    this.un_DocumentsViewStateChange = PubSub.subscribe(
      "DocumentsViewStateChange",
      this.onDocumentsViewStateChange.bind(this)
    );
    this.un_SetSelectedDocument = PubSub.subscribe(
      "SetSelectedDocument",
      this.onDocumentSelected.bind(this)
    );
    this.un_ShowDocument = PubSub.subscribe(
      "ShowDocument",
      this.onShowDocument.bind(this)
    );
    this.un_UnSelectDocument = PubSub.subscribe(
      "DocumentUnSelected",
      this.onDocumentUnSelected.bind(this)
    );
      ...
      leftPanel = (
        <div>
          ...
          <RepresentationDetails
            selected_document={this.state.selected_document}
            newfilename={this.state.new_file_name}
      ...
      <div className="caia-fill caia-background">
        ...
        >
          ...
          <button
            ...
            onClick={() => {
              this.UploadFileFieldRef.current.click();
            }}
          >
          ...
          <input
            id="file-input"
            type="file"
            ref={this.UploadFileFieldRef}
            className="invisible"
            onChange={this.onFileSelectionChangeHandler}
          />
        </Container>
      </div>
    );
  ...
  init() {
    ...
    let lastUpdate = ReactSession.get(
      "project_documents_lastime_pid" + this.project_id
    );
