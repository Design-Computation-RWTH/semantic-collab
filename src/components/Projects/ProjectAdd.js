...
export default function ProjectAdd({
  ...
  function CustomToggle({ OnClick, children, eventKey }) {
    const decoratedOnClick = useAccordionButton(eventKey, () => {
      isOpen = !isOpen;
      if (isOpen) OnClick("Cancel");
      else OnClick("Add a project");
    });
    if (!isOpen)
      return (
  ...
  function submitted(event) {
    ...
    this.bcfowl_setup.addProject(this.state.new_project_name).then(() => {
      ...
      PubSub.publish("Update", {
        txt: "Project created. Name: " + this.state.new_project_name,
      });
      projectList.update();
    });
  }
  ...
  return (
    <Accordion>
      <Card>
        <Card.Header>
          <CustomToggle OnClick={() => {}} eventKey="0">
            Add new Project
          </CustomToggle>
        </Card.Header>
        <Accordion.Collapse eventKey="0">
          <Card.Body>
            <Form onSubmit={submitted}>
              ...
              <CustomInternalToggle
                OnClick={(txt) => this.setState({ add_button_text: txt })}
                eventKey="0"
              >
